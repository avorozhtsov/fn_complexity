...
not transitive, and how large that failure is.  One theorem here --- on the
square class of a quadratic form --- is arithmetic the signature rate provably
cannot see.  Every numbered claim is marked
...
This looks like a liability, and \S\ref{sub:isotropy} shows it is the
opposite: the difference between the two rates is exactly where the arithmetic
that $C$ cannot see is kept.
...
\subsection{What the affine rate reads, and the signature rate cannot}
\label{sub:isotropy}

The objection above is fatal to $C$ and does not touch $\caff$.  The affine rate
is not a function of the signature (\S1), so nothing forces its arithmetic
content to be recoverable from $\{N_c\}$ --- and it is not.

\begin{theorem}[proved]\label{thm:isotropy}
\source{affine\_rate/ISOTROPY}
Let $Q$ be a binary quadratic form over any field $K$ with
$\operatorname{char}K \neq 2$.  Then
\[
  N_k(Q\ra x) = \bigl\lceil \tfrac{3k}{2} \bigr\rceil,\quad
  \caff(Q\ra x)=\tfrac23 \qquad\text{if $Q$ is anisotropic;}
\]
\[
  N_k(Q\ra x) = k,\quad \caff(Q\ra x)=1 \qquad\text{if $Q$ is isotropic.}
\]
\end{theorem}
...
quadratic part.  The statement therefore holds over $\mathbb Q_p$ and over $\R$
as well --- new for the seven anisotropic square classes of the $2$-adic
quadratic-map poset --- and it puts the value $2/3$ on a Hasse arrow the
finite-field posets record as missing.

\begin{corollary}[proved]\label{cor:congruence}
Over $\F_q$, $\caff(x^2+y^2 \ra x) = 2/3$ when $q \equiv 3 \bmod 4$ and $=1$
when $q \equiv 1 \bmod 4$.  The uniformly anisotropic family $x^2-ny^2$ ($n$ a
non-square) has $\caff(\cdot \ra x) = 2/3$ at every odd $q$.
\end{corollary}

So $\caff$ jumps by exactly $1/3$ across a congruence: it detects the
\textbf{square class of the discriminant}, a genuine arithmetic invariant.  What
$C$ does on the same pair is the point.
...
& $q=3$ & $q=47$ & behaviour \\
\midrule
$\caff$ separation of the two classes & $1/3$ & $1/3$ & constant \\
$C$ separation of the same two classes & $0.0087$ & $0.0014$ & $\to 0$ \\
amplification $\caff/C$ & $38$ & $245$ & grows in $q$ \\
\bottomrule
\end{tabular}
\end{center}

$C$ moves in the right direction --- an isotropic form is the easier resource
--- but its separation \emph{shrinks} as $q$ grows, while $\caff$'s is exactly
$1/3$ for every $q$.  The signature sees $(4,4,1)$ against $(5,2,2)$ and barely
flinches; the affine rate sees a hyperbolic plane against an anisotropic form
and jumps.

\begin{remark}[what this does and does not answer]
This is the one place in the programme where the exchange rate carries
arithmetic that is not a monotone relabelling of a classical statistic of the
same object, and the reason is structural: $\caff$ is a functional of the
\emph{maps}, and the signature is a lossy quotient of them.  It does not rescue
the propositions above, which remain translations of $\max_c N_c$ and $m_2$.  It
relocates the claim: the arithmetic content of this framework lives in $\caff$,
and every computation in this project other than Theorem~\ref{thm:isotropy} has
been performed on $C$.
\end{remark}
...
$\caff$.  On the five non-constant quadratic classes over $\F_3$ the tally is now
$13$ pairs with $\caff < C$, $4$ with $\caff > C$ (all one-step implementable,
where $\caff = 1$ is forced), $6$ equal at $1$, and $2$ undetermined --- so
\emph{off the implementable locus $\caff \le C$ holds wherever it is known}, a
statement equivalent to the open question whether $\caff = 1$ is possible
without $f \impl_{\mathrm{aff}} g$.

\paragraph{Rationality.}
Every $\caff$ proved here comes from an $N_k$ that is exactly linear or exactly
$\lceil 3k/2 \rceil$, hence eventually exactly linear along arithmetic
progressions.  Whether that is general is \textbf{open}, and is the honest
analogue of the rationality of the matrix-multiplication exponent:
$\caff = 1/\lim_k (N_k/k)$ is a limit of rationals with denominator $k$ and need
not be rational a priori.
...
operational; the last two affine brackets over $\F_3$, both with an isotropic
resource, now reduced to a single conjecture on the excess of a span of
rank-$\le2$ forms; and whether $\caff$ separates distinct anisotropic square
classes over $\mathbb Q_p$, where the norm group has index $2$ and the
finite-field argument has no analogue.
