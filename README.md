...
[exchange_rates_finite_map_signatures.pdf](paper_exchange_rate/exchange_rates_finite_map_signatures.pdf).
The finite-field extension begins with the documented quadratic-map posets for
[`q = 2, 3, 4, 5, 8, 9, 11, 13, 16, 25`](paper_finite_fields_maps/docs/quadratic_map_posets.md).
The same affine-degeneration order over
[`R` and `C`](paper_finite_fields_maps/docs/real_complex_quadratic_map_posets.md)
is shown in two hand-authored comparison diagrams.
A richer hand-authored example over the
[`2`-adic numbers](paper_finite_fields_maps/docs/p_adic_quadratic_map_poset.md)
separates seven anisotropic square classes.
The accompanying notes distinguish
[local zeta-profile currency](paper_finite_fields_maps/docs/p_adic_zeta_exchange_currency.md)
from operational affine rates and record the exact
[Weil exchange-matrix reformulation of the Riemann hypothesis](paper_finite_fields_maps/docs/riemann_hypothesis_exchange_matrices.md).
The first cubic comparison, for \(\mathbb F_3^2\to\mathbb F_3\), contrasts
[affine input processors with the preorder generated by quadratic input processors](paper_finite_fields_maps/docs/cubic_map_posets_q3.md).
The one pair of nondegenerate binary forms with a proved fractional affine
rate is `x^2+y^2` against `x^2-y^2`: the inertia monotone gives
`C_aff(x^2-y^2 -> x^2+y^2) = 1/2` exactly, while an explicit three-copies-for-two
certificate gives `C_aff(x^2+y^2 -> x^2-y^2) >= 2/3`, so the pair is certified
one-way.  Both are checked by
[`analysis/definite_hyperbolic_affine_rates.py`](analysis/definite_hyperbolic_affine_rates.py).
